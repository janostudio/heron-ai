...
| **执行中** | 当前 Team 主 A / Agent | 追加指令，继续执行，不中断 |
| **Ctrl+C** | 全局主 A | 中断当前执行 → 回到主 A |

### 规则

1. **空闲时**输入给主 A——没有正在执行的任务，主 A 负责规划调度
2. **执行中**输入给当前执行的 Team 主 A 或 Agent——用户看到 "researcher 正在搜索..."，输入 "查下 2024 年数据"，追加指令继续执行
3. **Ctrl+C** 回到主 A——用户想换方向，中断当前任务，重新决策

### 不需要前缀

用户不需要加 `/tell` 或 `/new` 前缀，引擎根据状态自动路由：

```
空闲：  "帮我写博客"     → 主 A → dispatch blog_team
执行中："加个案例"       → 当前 Team 主 A → 追加指令给 writer
Ctrl+C："算了，查天气"   → 主 A → 中断 → 重新决策
```

### 执行过程可见

用户能看到当前哪个 Team/Agent 在输出（这是优化点——当前看不到），输入自然知道追加给谁。
